...
function readScript() {
    ...
    if (line.type === undefined) {
        ...
        dialog.textContent = '';
        conversation = formatScript(line.value.slice(2)).join('');
        conversationName = formatScript([line.value[1]]);
        console.log(conversation);
        let imgPath = line.value[0];
        document.querySelector('img#character').src=(imgPath==''?'blank.png':`${game.url}/${imgPath}`);
...
async function startGame() {
    ...
    document.body.innerHTML = `<img id='character' src='blank.png'><div class='choices'></div><div id="boxes"><div id="name-box"></div><p id="name-label"></p><textarea id="dialog-box"></textarea></div><button id='next' onclick="stopScript()"></button><img id="bg" />`;
